# Auth / Firebase boot practices (#850 / #858 / #880)

Standing rules for app-document auth and any surface that opens Google/`signInWithPopup` or touches Firestore under App Check. Learned from the v1.48.4 Safari hotfix (#850), Tier 0 hard-ready Google CTA (#858), and T2.5 Phase A speculative idle warm (#880); apply on invite/join and further `/login` feel work without re-deriving.

**Related:** `docs/RELEASE_TRAIN_COLD_OPEN.md` · `docs/OUTBOUND_AUTH_HANDOFF.md` · `docs/API.md` (`/login`) · **roadmap / top-tier gap close:** `docs/AUTH_SEAMLESS_PATH.md` · code: `src/shared/lib/ensureFirebase.js`, `src/features/auth/model/warmLoginAuthSurface.js`, `src/features/landing/model/prefetchLoginIntent.js`

---

## Rules

1. **Marketing `/` stays Firebase-free on cold first paint.** Second Vite entry (`index.html` → `marketingMain`). Auth CTAs hard-navigate to `/login` (app document). Do not put `AuthProvider` / `initializeApp` on the marketing critical path. **After idle (#880):** download-only prefetch of `/login` + login UI + `firebase-core` is allowed (no execute on marketing). See `AUTH_SEAMLESS_PATH.md` §4 Tier 2.5 / §8.
