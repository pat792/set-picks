...
| **Standings** | Tab, context bar, and desktop H1 for `/dashboard/standings` (`NAV_LABEL_STANDINGS`) — aligned so the shell does not repeat two headings. In-page surface opens with Invite + Scoring rules, then a three-way pill toggle **Show / Tour / Pools** (#255); view is URL-synced via `?view=show\|tour\|pools` and the Pools view takes an optional `?pool=<id>` sub-selector. `?view=tour` hides the global date picker (tour standings are cumulative). (Sticky Standings chrome remains staging-only until Sprint 9.) |
