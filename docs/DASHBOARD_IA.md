...
| **Standings** | Tab, context bar, and desktop H1 for `/dashboard/standings` (`NAV_LABEL_STANDINGS`) — aligned so the shell does not repeat two headings. Utility row above **Show / Tour / Pools**: **Invite friends** with Tour Date–style white→teal gradient outline (left) + neutral **Scoring rules** GhostPill matching Picks (right). Solid teal fill stays reserved for view selection. Empty Show/Pools boards also surface an Invite CTA. View is URL-synced via `?view=show\|tour\|pools` and the Pools view takes an optional `?pool=<id>` sub-selector. `?view=tour` hides the global date picker (tour standings are cumulative). (Sticky Standings chrome remains staging-only until Sprint 9.) |
...
| **Last show's winner / winners** | Standings callout for the **prior tour night** when the selected show is **NEXT** or **LIVE** (#218, #305). **Show tab:** global prior-night picks; **Pools tab:** pool-filtered prior night (same rules as tonight for that audience). Heading uses `lastShowWinnerHeading(winnerCount, poolName?)` — on Pools, suffix **`· {pool name}`** when known. |
