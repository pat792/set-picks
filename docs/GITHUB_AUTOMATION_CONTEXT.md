...
- **Public landing SEO:** `src/shared/config/seo.js` + `seoRoutes.js` (prerender registry), `og-home-html.mjs` + `middleware.js` (social scrapers only on `/` — not Googlebot/bingbot), post-build `scripts/prerender-seo.mjs` (`/`, `/how-it-works`, `/how-scoring-works`), `src/features/landing/ui/LandingSeo.jsx` (Helmet + JSON-LD), `public/llms.txt`, `public/branding/og-card-1200x630.jpg` (regenerate via `npm run generate:og-card` after changing splash SVGs; bump `OG_IMAGE_VERSION` in `seo.js` / `og-home-html.mjs`). Ops + GSC baseline: **`docs/SEO_GEO_PLAYBOOK.md`** (epic #657 / #664).
