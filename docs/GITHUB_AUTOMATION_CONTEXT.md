...
- **SEO GSC weekly pack (#932 / epic #926):** `.github/workflows/seo-gsc-weekly-schedule.yml` posts a `[SKIP-PRD]` **SEO Optimize pack** comment on #926 (last-7d GSC registry queries + top new queries + GA4 organic landings for `/tour-stats*` and `/phish-setlist-prediction-game`). Script: `scripts/seo/gsc-weekly-snapshot.mjs`. **Does not** merge, deploy, or open PRs. Posts only when live GSC credentials are present; otherwise SKIP (fork-safe). Secrets: see playbook §4 + table below. Scored tune loop (draft PR to `staging` only when justified): **`docs/SEO_OPTIMIZE_AUTONOMY.md`** (#934). No weekly Action opens PRs.
...
- **Public landing SEO:** `src/shared/config/seo.js` + `seoRoutes.js` (prerender registry), `og-home-html.mjs` + `middleware.js` (social scrapers only on `/` — not Googlebot/bingbot), post-build `scripts/prerender-seo.mjs` (tour-stats SEO auto-expand #959 from `public_tour_stats/_index`), `src/features/landing/ui/LandingSeo.jsx` (Helmet + JSON-LD), `public/llms.txt`, `public/robots.txt`, `npm run verify:seo-strings` / `verify:og-home` / `verify:seo-prerender`, `public/branding/og-card-1200x630.jpg` (regenerate via `npm run generate:og-card` after changing splash SVGs; bump `OG_IMAGE_VERSION` in `seo.js` / `og-home-html.mjs`). Ops + GSC baseline: **`docs/SEO_GEO_PLAYBOOK.md`** (epic #657 / #664; Child D #930 / E0 #931 / E1 #932 / E3 #934 / auto-expand #959 / epic #926). Query registry: `docs/seo/query-registry.json`. Weekly facts packs: `npm run seo:gsc-weekly-snapshot`. Scored loop: **`docs/SEO_OPTIMIZE_AUTONOMY.md`**. Public profiles `/user/:userId` are **`noindex,follow`** (#661).
