...
- **Inbound (v1.75.0):** `commsResendInboundWebhook` handles `email.received` on a **separate** URL. Allowlist `updates@` / `unsubscribe@` → `emails.receiving.forward` to `support@road2media.com`. Catch-all other local-parts return 200. [INBOUND_FORWARDING.md](./INBOUND_FORWARDING.md).
