...
| `/tour-stats` | None | **v1.33.0 (#665):** public aggregate tour song stats (filter + default Sphere). Prerendered shell; live data from `public_tour_stats`. Never full nightly setlists. **v1.47.0:** remains on the **app** document (Firestore). **v1.47.1 (#827):** boot warms App Check immediately; prerender `modulepreload`s `PublicTourStatsPage`; loading UI keeps skeleton chrome (not a blank spinner). |
| `/tour-stats/:tourSlug` | None | **v1.33.0 (#665):** same surface for a kebab-case tour slug (e.g. `2026-sphere`). Default Sphere slug is also prerendered. **v1.47.1 (#827):** same boot / preload / skeleton behavior as `/tour-stats`. |
