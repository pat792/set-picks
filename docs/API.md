...
**Picks — Make Picks song picker (**v1.71.0 / #1013**):** the autocomplete dropdown shows trailing compact model odds (`N%`, or `<1%` / mobile `<1` when the title is missing from `playProbBySong`) from the same Storage artifact as Scorecard, only when `targetShow.date` matches the selected night. Total / Gap / Last are unchanged. Not live crowd %.
