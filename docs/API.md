...
| `/login` | None | **v1.47.0 (#832):** authenticated-SPA auth entry (`app.html`). Hosts splash sign-in / create-account modals. Marketing CTAs hard-navigate here. Compatibility: `/?login=true` on the app or marketing document redirects here. **v1.47.3 (#830):** unauthenticated `/dashboard/*` and `/setup` hard opens bounce here (not marketing `/`); outbound inventory in `docs/OUTBOUND_AUTH_HANDOFF.md`. |
