...
**Status:** EiC-approved L0 (2026-08-21) — **this PR is docs-only**  
**Date:** 2026-08-21 (relanded on staging 2026-09-02)  
**Epic:** #972  
...
Next/A execution is kicked off via sibling issues (#973 title/H1, #974 show-week + always-on social, #975 gated `/phish-picks`). **Do not implement those in this PR.** Child E0 query registry is #931 (sibling).
...
**Already shipped:** Summer tour SEO URL (#927), crawlable bustout/frequency facts (#928), fan-language H2s (#929), GEO brief + playbook kickstart (#930).
...
Stable IDs — keep forever once logged. Playbook §3 is the live ops table; **do not renumber** IDs already used there (Child D / #930 added S4–S6). The 2026-08 draft’s original S4 phrase (`phish unique songs` / `songs played this tour`) maps to playbook **S6**.
...
| S4 | `bustouts summer tour 2026` | summer slug bustouts | P1 (#930) |
| S5 | `phish bustout list this tour` | summer slug bustouts | P1 (#930) |
| S6 | `phish unique songs` / `unique songs played this tour` / `songs played this tour` | summer slug | P2 |
...
|-------|--------|-------|------------|----------|
| **0 — Now** | Plan + decisions on disk | #972 / #970 | — | This doc (**docs PR; no product code**) |
| **1 — Meta bridge** | Title/H1/FAQ for C6/C7 | #973 | Kickoff | Copy PR → `staging` (**sibling; not this PR**) |
| **2 — Index & verify** | GSC summer tour-stats; bustout HTML | #930 (reuse) | #927–#929 live | Playbook §2 |
| **3 — Social authority** | Owned IG/X + show-week **and** always-on packs | #974 | Accounts + EiC | `crew/output/demand_gen/` (**sibling**) |
| **4 — Optimize engine** | Registry + weekly packs + competitor briefs | #931–#934 under #926 | — | Packs on #926 (**#931 sibling**) |
| **5 — Gated doorway** | `/phish-picks` only if overtake evidence | #975 | Gate checklist | **No build until green** (**sibling**) |
...
- Implementing #973 / #974 / #975 / #931 in the plan/decisions PR.
...
**Resolved 2026-08-21** — see Decisions table at top.

**2026-09-02:** Plan + decisions relanded on current `staging` (docs-only). Next/A execution is kicked off via sibling issues; this file stays the north-star artifact.

Remaining before / during execution:

1. Create owned IG (+ optional X/Threads) accounts.
2. Explicit **kickoff** per child (#973 / #974 / #930 remainders) — no silent starts.
