...
export default function DashboardMobileContextBar({
  ...
  return (
    <div
      className={`absolute top-full left-0 w-full bg-surface-chrome/95 backdrop-blur-xl supports-[backdrop-filter]:backdrop-saturate-150 border-b border-border-subtle/35 shadow-[0_10px_24px_-18px_rgba(15,10,46,1)] px-4 py-3 flex flex-row flex-nowrap items-center justify-between gap-2 min-w-0 transition-transform duration-300 ease-in-out z-10 ${
      ...
      {showDatePicker && (
        <div className="flex shrink-0 flex-row flex-nowrap items-center gap-1.5 min-w-0">
          ...
          <select
            ...
            className="show-date-select shrink-0 max-w-[180px] min-w-0 appearance-none bg-surface-field border border-border-subtle/45 text-white text-xs font-bold py-1.5 px-2.5 rounded-lg outline-none focus:border-teal-400 transition-colors cursor-pointer"
          >
