...
export default function App() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <button 
        ...
        className="bg-white text-black px-10 py-4 rounded-full font-black flex items-center gap-3 shadow-xl hover:scale-105 transition-all"
      >
  ...
  if (!userProfile) return (
    <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 w-full max-w-sm space-y-4 shadow-2xl">
        ...
        <button onClick={() => saveHandle(document.getElementById('handleInput').value)} className="w-full bg-blue-600 py-4 rounded-xl font-black hover:bg-blue-500 transition-colors">START PLAYING</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-blue-500/30">
      ...
      {/* BOTTOM NAVIGATION (Forced to the front) */}
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-2xl border border-white/10 rounded-full p-2 flex gap-1 z-[70] shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        {[
      ...
      <div className={`fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[100] transition-opacity duration-300 ${isMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`} onClick={() => setIsMenuOpen(false)}>
        <div 
          className={`absolute right-0 top-0 h-full w-80 bg-slate-800 shadow-[ -20px_0_50px_rgba(0,0,0,0.3) ] p-8 flex flex-col transform transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}
        ...
        >
          ...
          <button onClick={() => signOut(auth)} className="w-full py-5 bg-slate-900 rounded-2xl text-xs font-black uppercase tracking-[0.2em] text-red-400 border border-red-900/20 hover:bg-red-900/10 transition-colors">Sign Out</button>
        </div>
      </div>
    </div>
  );
