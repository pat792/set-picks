...
export default function App() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <button onClick={async () => { try { await signInWithPopup(auth, googleProvider); } catch (e) { alert(`Login Error: ${e.code}`); } }} className="bg-white text-black px-10 py-4 rounded-full font-black flex items-center gap-3 shadow-xl hover:scale-105 transition-all">
  ...
  if (!userProfile) return (
    <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 w-full max-w-sm space-y-4">
        <h2 className="text-2xl font-black text-white">Choose Your Handle</h2>
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-blue-500/30">
      <Header selectedDate={selectedDate} setSelectedDate={setSelectedDate} activePoolName="Global Pool" onOpenMenu={() => setIsMenuOpen(true)} />
      ...
      <main className="max-w-xl mx-auto px-6 pb-48 pt-8">
        <div className="flex bg-slate-900/50 p-1 rounded-2xl mb-8 border border-slate-700 shadow-inner">
          <button onClick={() => setActiveTab("picks")} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'picks' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>Picks</button>
          <button onClick={() => setActiveTab("pools")} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'pools' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>Leaderboard</button>
          {user.email === ADMIN_EMAIL && <button onClick={() => setActiveTab("admin")} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'admin' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>Admin</button>}
        </div>
        ...
        {activeTab === "admin" && user.email === ADMIN_EMAIL && (
          <div className="space-y-6">
            <div className="bg-slate-800/80 p-6 rounded-[2.5rem] border border-emerald-500/30 space-y-5 shadow-xl">
              ...
              <button onClick={handleSaveResults} className="w-full bg-emerald-600 py-5 rounded-3xl font-black tracking-widest text-black hover:bg-emerald-500 transition-colors mt-4">{saveStatus || "PUBLISH RESULTS"}</button>
            </div>
          </div>
        )}
      </main>
      ...
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-2xl border border-white/10 rounded-full p-2 flex gap-1 z-[70] shadow-2xl">
        {[{ id: "picks", label: "Picks", icon: "🎟️" }, { id: "pools", label: "Pools", icon: "🤝" }, ...(user?.email === ADMIN_EMAIL ? [{ id: "admin", label: "Admin", icon: "👑" }] : [])].map(t => (
