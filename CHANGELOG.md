...
### Added
- **Mobile Chrome keyboard / Make Picks chrome investigation (#1041)** — `docs/investigations/mobile-chrome-keyboard-picks-chrome.md` diagnoses iOS Chrome letterboxing vs usable Safari on the same form (evidence under `docs/investigations/evidence/`). Ranks fix options (hide nav while typing, picks entry focus mode, portaled autocomplete). Docs only; no version bump.
