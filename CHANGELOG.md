...
### Added
- **Mobile Chrome keyboard / Make Picks chrome investigation (#1041)** — `docs/investigations/mobile-chrome-keyboard-picks-chrome.md` diagnoses iOS Chrome letterboxing (fixed bottom nav above keyboard + tertiary stack height) and ranks fix options (hide nav while typing, picks entry focus mode, portaled autocomplete). Docs only; no version bump.
