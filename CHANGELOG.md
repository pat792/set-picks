...
- **Tour Date scope stepper (#609)** — single-row Tour Date control with prev/next arrows (`ChromeScopeStepper` + `DashboardTourDateScope`) on mobile context bar and desktop sticky chrome. Label stays inline (not stacked) so row height matches the prior chrome. Tour scope stays a single-row select without arrows (only ~2 selectable tours today; revisit when the list regularly exceeds ~3).
