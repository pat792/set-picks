...
- **Tour Date scope stepper (#609)** — centered Tour Date control with prev/next arrows (`ChromeScopeStepper` + `DashboardTourDateScope`) on mobile context bar and desktop sticky chrome for seamless night-to-night switching. Tour scope stays a centered select without arrows (only ~2 selectable tours today; revisit when the list regularly exceeds ~3).
